...
def processATF(excelfile, output_path, now_date):
    ...
    for row in range(start_row + 1, end_row + 1):
        ...
        if any([
            ...
            print("\tATF REJECTED. REASON : ATF CONTAINS BLANK INFORMATION")
            rejected_tasks.append((task_id_destination, "DATA ON EXCEL CONTAINS BLANK INFORMATION"))
            continue

        serial_number_atf = str(sheet['G' + str(row)].value).strip()

        print("\tPROCESSING (%s/%s): SN %s of task %s" % (row - 1, end_row + 1, serial_number_atf, task_id_destination))

        task_id_origin = queries.getOriginTaskData(task_id_destination)

        if not task_id_origin:
            rejected_tasks.append((task_id_destination, "TASK ID DESTINATION DOES NOT EXIST"))
        ...
        fri_filepath = task_id_folder + "\\" + "%s_%s_%s.fri" % (task_id_excess, site_id_fri, serial_number)
        ...
        tbl_item = (
            major_equipment, manufacturing_atf, equipment_category_atf, equipment_type_atf, unit_category,
            serial_number, unit_of_measurement_atf, req_qty, flag_waste_atf, now_date, movement_flag,
            mobile_defined_status, task_id_excess, item_id
        )
        ...
        data_atf_to_record = (
            ne_code, major_equipment, manufacturing_atf, equipment_category_atf, equipment_type_atf, unit_category,
        ...
        data_netgear_to_record = (
            ...
            task_id_excess, os.path.basename(excelfile)
        )
    ...
    for tid in task_id_destination_list:
        ...
        task_origin_data = (
            tie, project_id, site_id_origin, plan_date, status_origin,
            remark, task_type, last_update, vendor, ne_id,
            atf_no, division, site_id_destination, creator_id
        )
...
if __name__ == '__main__':
    ...
    input_path = "D:\\KERJAAN\\SCRIPTS\\SCRIPTS\\Made by AMG\\MANUAL - NETgear ATF Uploader\\INPUT\\EXCESS"
    list_of_atf_excess = glob.glob("%s\\*.xlsx" % input_path)

    output_path = "D:\\KERJAAN\\SCRIPTS\\SCRIPTS\\Made by AMG\\MANUAL - NETgear ATF Uploader\\OUTPUT\\EXCESS"
    output_path = output_path + '\\' + batch_name
    checkDir(output_path)

    done_path = "D:\\KERJAAN\\SCRIPTS\\SCRIPTS\\Made by AMG\\MANUAL - NETgear ATF Uploader\\DONE\\EXCESS"
    done_path = done_path + '\\' + batch_name
    checkDir(done_path)

    material_path = "D:\\KERJAAN\\SCRIPTS\\SCRIPTS\\Made by AMG\\MANUAL - NETgear ATF Uploader\\MATERIALS\\EXCESS"
