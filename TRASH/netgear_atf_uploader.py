...
def processATF(excelfile, output_path, now_date):
    ...
    for row in range(start_row + 1, end_row):
        ...
        exception_serialnumbers_in = ['broken', 'blank', 'none', 'terbaca']
    ...
    vendor = str(sheet['E18'].value).strip()
    atf_no = str(sheet['H5'].value).strip()

    division = queries.getDivision(vendor)[0][0] if 'mpfs' not in project_id.lower() else \
        queries.getDivision('mpfs' + vendor)[0][0]
